...
import { Award, Lock, Palette, Eye, Edit2, Check, Sparkles, Sliders, MessageSquare, History, Trophy, Flame, ShieldAlert, Users, ChefHat, Heart } from "lucide-react";
...
export default function ProfileScreen({ user, onChangeUser, onNavigate, onManageProfiles, onSignOut }: ProfileScreenProps) {
  ...
  return (
    <div className="relative min-h-[500px]">
      ...
      <div className={`space-y-8 animate-fade-in text-slate-800 pb-12 ${user.isGuest ? "blur-md select-none pointer-events-none opacity-40 transition-all duration-300" : ""}`}>
      ...
      <div className="bg-white border border-slate-200 rounded-3xl p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm">
        <div className="flex flex-col md:flex-row items-center gap-5 text-center md:text-left">
          ...
          <div className="space-y-1.5">
            ...
            {onSignOut && !user.isGuest && (
              <div className="flex gap-2 pt-2.5 justify-center md:justify-start font-mono uppercase text-[10px]">
                <button
                  type="button"
                  onClick={onSignOut}
                  className="px-3 py-1.5 bg-slate-100 hover:bg-slate-250/20 text-slate-700 border border-slate-200 rounded-xl font-black flex items-center gap-1 cursor-pointer transition-all active:scale-98"
                ...
                <button
                  type="button"
                  onClick={onSignOut}
                  className="px-3 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 rounded-xl font-black flex items-center gap-1 cursor-pointer transition-all active:scale-98"
                  title="Disconnect Current Chef Profile"
                >
                  <span>Sign Out</span>
                </button>
              </div>
                  ...
                  return (
                    ...
                    >
                      <div className={`w-14 h-14 rounded-full flex items-center justify-center shadow-sm ${
                        ...
                        {unlocked ? (
                          badge.id === "first_steps" ? <ChefHat className="w-6 h-6" /> :
                          badge.id === "steady_cook" ? <Flame className="w-6 h-6" /> :
                          badge.id === "perfect_tempo" ? <Award className="w-6 h-6" /> :
                          badge.id === "expert_plates" ? <Trophy className="w-6 h-6" /> :
                          badge.id === "flawless_run" ? <Sparkles className="w-6 h-6" /> :
                    ...
                    >
                      ...
                      <div className="grid grid-cols-3 gap-2 text-center text-[10px] font-mono bg-white border border-slate-150 rounded-xl p-2 font-bold text-slate-700">
                        ...
                        <div>
                          ...
                          <span>{session.timerAccuracyBonus ? "Earned Bonus" : "Missed"}</span>
                        </div>
                      </div>
      ...
      {/* MULTI-USER PROFILES MANAGER CARD */}
      {onManageProfiles && (
        <div className="bg-gradient-to-tr from-[#FAF9F6] to-[#F1EDE4]/35 border border-slate-200 rounded-3xl p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-6 shadow-xs">
          <div className="space-y-1 text-center md:text-left">
            <span className="text-[10px] font-mono font-black text-[#A0855B] block tracking-widest uppercase flex items-center gap-1.5 justify-center md:justify-start">
              <Users className="w-3.5 h-3.5 text-[#A0855B]" /> Multi-Chef Workspace
            </span>
            <h4 className="text-sm font-extrabold text-slate-800 leading-snug font-serif">Sign Up & Toggle Chef Profiles</h4>
            <p className="text-xs text-slate-500 max-w-xl">
              Add separate profiles for family members, friends, or wellness targets. Keep day streaks, kitchen pantry shelves, and cooking XP fully isolated.
            </p>
          </div>
          <button
            onClick={onManageProfiles}
            className="px-5 py-2.5 bg-slate-900 text-amber-400 hover:bg-slate-800 rounded-xl text-xs font-bold uppercase tracking-wider transition-all cursor-pointer shadow-sm flex items-center gap-2"
          >
            <Sparkles className="w-3.5 h-3.5" />
            <span>Manage Profiles / Sign Up</span>
          </button>
        </div>
      )}
      ...
      {user.isGuest && (
        <div className="absolute inset-x-0 top-16 z-50 flex items-center justify-center p-4">
          <div className="bg-white/95 border border-[#A0855B]/30 rounded-3xl p-8 max-w-sm w-full shadow-2xl text-center space-y-6 animate-fade-in border-t-4 border-t-[#A0855B]">
            <div className="w-16 h-16 bg-amber-50 rounded-full flex items-center justify-center mx-auto border border-amber-100 shadow-sm">
              <Lock className="w-7 h-7 text-[#A0855B]" />
            </div>
            
            <div className="space-y-2">
              <h3 className="text-lg font-serif font-black text-slate-950">Unlock Chef Profile</h3>
              <p className="text-xs text-slate-500 leading-relaxed font-semibold">
                Guest logs do not track statistics or custom setups. Build your real chef account to store favorites, check achievements, and swap premium workspaces!
              </p>
            </div>

            <div className="space-y-4 pt-1">
              {/* TOP: "add a sign up button on the top" */}
              <button
                type="button"
                onClick={() => {
                ...
                className="w-full py-3.5 bg-slate-900 hover:bg-slate-850 text-amber-300 font-bold rounded-2xl text-xs uppercase tracking-wider shadow-md transition-all active:scale-98 cursor-pointer flex items-center justify-center gap-1.5"
              >
                <Sparkles className="w-4 h-4 text-amber-300 animate-pulse" />
                <span>Sign Up Now</span>
              </button>

              {/* BOTTOM: "and under there add 'already have an account?' text which is blue and underlined and is a link that leads to the login page" */}
              <div className="text-center pt-1">
                <button
                  type="button"
                  onClick={() => {
                    localStorage.setItem("cook_mate_land_mode", "login");
                    if (onSignOut) onSignOut();
                  }}
                  className="text-xs font-bold text-blue-600 hover:text-blue-800 underline hover:no-underline cursor-pointer bg-transparent border-none p-0 outline-none transition-colors"
                >
